 ...
 * DevOps extension does — camelCase (`connectionType`, `filesDeployed`,
 * `outliersCount`) — because that is the vocabulary the shared task runners
 * were written in. `action.yml` spells the same things in kebab-case, so every
 * name crossing this boundary goes through {@link toActionInputName}.
 *
 ...
 * consuming workflow simply reads an empty string.
 * `src/lib/__tests__/github-task-host.test.ts` asserts against real
 * `$GITHUB_OUTPUT` / `$GITHUB_ENV` files for exactly this reason — a mocked
 * `@actions/core` cannot tell the two calls apart.
 */
