...
readable source. Worse, the `--source-map` build emitted an `index.js.map`
whose `sourcesContent` embedded a second, *fully readable* copy of every
bundled dependency — 60 symitar files among them. Minifying the bundle does
nothing about the map, so the build drops both: `--minify`, no `--source-map`,
and the post-build sweep deletes any `.js.map` and `sourcemap-register.js` that
reappear.

Be honest about what this buys: obfuscation, not encryption. Exported class
names still appear. It removes comments, structure and the verbatim source
dump — the difference between "published" and "recoverable with effort". The
commercial protection is the licence check, not the minifier.

Consequences:

- **Stack traces point into the bundle**, not into source. `main.ts` logs stacks
  at debug level anyway.
...
- "Assert the bundle ships minified and mapless" fails the build if the map
  returns or `--minify` is dropped, using a bytes-per-line ratio: ~33
  unminified, ~415,000 minified.

`validate-poweron-action` is configured identically; keep the two in step.
